...
# REVIEW [Coding standards]: This function (and build_vocab above) uses print() for progress
# output. That's fine for the __main__ demo at the bottom of the file, but inside a reusable
# data-pipeline function it means callers can't redirect/silence/level this output (no way to
# turn it off in a notebook or a training script that imports this module). Consider Python's
# `logging` module instead, so verbosity becomes configurable by the caller.
